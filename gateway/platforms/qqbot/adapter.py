...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._group_read_users = _coerce_list(extra.get("group_read_users") or extra.get("groupReadUsers"))
    ...
    _INBOUND_HANDLERS = {
        ...
        "GROUP_MESSAGE_CREATE": "_handle_group_message",
    ...
    async def _send_text_to(
        ...
        # 重要修复：群消息不把 reply_to 写进 body["msg_id"]。QQ Bot API 对带
        # msg_id（消息引用）的群消息存在可见性 Bug：当被引用的消息来自非机器人
        # owner 的普通成员时，QQ 端不会把该回复投递给消息引用链上的普通成员，
        # 导致对方看不到回复。去掉 msg_id 后改为纯群广播，全员可见（实测验证）。
        # c2c 私聊保留引用（无此问题）。若未来 QQ 修复该行为，可恢复 msg_id。
        if reply_to and chat_type != "group":
